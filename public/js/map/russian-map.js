...
var RussianMap = function(options, regions) {
    ...
    // var existOrganizationMoveAttr = options.existOrganizationMoveAttr || {
    //     fill: '#d96c3a'
    // };
    var existOrganizationMoveAttr = options.defaultAttr || {
        fill: '#d8d8d8', // цвет которым закрашивать
        stroke: '#ffffff', // цвет границы
        'stroke-width': 1, // ширина границы
        'stroke-linejoin': 'round' // скруглять углы
        ...
        if (region.paths !== undefined) {
            for (var p in region.paths) {
                if(region.organization)
                    region.paths[p].attr(region.attr || existOrganizationMoveAttr);
                else
    ...
    var renderRegion = function(region) {
        ...
        if (region.paths !== undefined && region.paths !== null) {
            for (var p in region.paths) {
                var path;
                if(region.organization){
                    path = R.path('M' + region.paths[p]).attr(region.attr || existOrganizationMoveAttr);
                }
                else {
                    path = R.path('M' + region.paths[p]).attr(region.attr || defaultAttr);
                }
        ...
        if (region.polygons !== undefined && region.polygons !== null) {
            for (var p in region.polygons) {
