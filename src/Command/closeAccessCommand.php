 ...
 * @CronJob("1 *\/1 * * 5")
 */
class closeAccessCommand extends Command
{

    protected static $defaultName = 'app:access:close';
    ...
    {
        $this
            ->setDescription('Закрыть доступ для заполнения закупок')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if(array_key_exists($hour, $schedule))
        {
            ...
            foreach ($regions as $region)
            {
                ...
                {
                    $user_info->setAccessToPurchases(false);
        ...
        $io->success(sprintf('Доступ закрыт для '.$open_count.' кластеров'.$hour));
