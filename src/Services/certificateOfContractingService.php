...
class certificateOfContractingService extends AbstractController
{
    ...
    public function generateSertificate($id, $today = null, $option)
    {
        ...
        if(in_array('ROLE_BAS', $user->getRoles()))
            $procent['contractRegionFunds'] = ($userInfo->getRegionFundsGrant() > 0) ? ($sum['contractRegionFunds'] * 100)/($userInfo->getRegionFundsGrant()) : 0;
        else
        ...
        if(in_array('ROLE_BAS', $user->getRoles()))
        {
            $procent['factRegionFunds'] = ($userInfo->getRegionFundsGrant() > 0) ? ($sum['factRegionFunds'] * 100)/($userInfo->getRegionFundsGrant()) : 0;
        }
        else
        {
